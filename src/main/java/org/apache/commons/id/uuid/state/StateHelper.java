...
public final class StateHelper implements Constants {
	...
	private static Log log = LogFactory.getLog(StateHelper.class);
    ...
    public static byte[] readNodeIdentifier() {
    	...
    	try {
			...
			if (mac == null) {
				...
				while (nics.hasMoreElements()) {
					...
					if (!nic.isLoopback() && nic.getHardwareAddress() != null) {
			...
			if (mac == null) {
				log.warn("Unable to determine node id.  Using random node id.");
				mac = randomNodeIdentifier();
			}
			return mac;
    	}
    	catch (Exception e) {
